...
describe LuckyRecord::Migrator::CreateForeignKeyStatement do
  it "generates correct sql with cascade strategy" do
    statement = LuckyRecord::Migrator::CreateForeignKeyStatement.new(:comments, :users, column: :author_id, on_delete: :cascade, primary_key: :uid).build
  ...
  it "generates correct sql with nullify strategy" do
    statement = LuckyRecord::Migrator::CreateForeignKeyStatement.new(:comments, :users, column: :author_id, on_delete: :nullify, primary_key: :uid).build
  ...
  it "raises error on invalid on_delete strategy" do
    expect_raises Exception, "on_delete: :cascad is not supported. Please use :do_nothing, :cascade, :restrict, or :nullify" do
      LuckyRecord::Migrator::CreateForeignKeyStatement.new(:comments, :users, on_delete: :cascad).build
    end
  end
end
