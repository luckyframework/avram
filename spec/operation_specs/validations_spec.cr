...
private class OperationWithValidations < Avram::Operation
...
describe Avram::Validations do
  ...
  describe "validate_size_of" do
    it "validates" do
      ...
      number_attribute = attribute(21)
      Avram::Validations.validate_size_of(number_attribute, min: 18, max: 22)
    end
  ...
  context "Operation with validations" do
    it "is valid when validations pass" do
      ...
      OperationWithValidations.run(params) do |operation, value|
        ...
        value.should eq "Make the tacos"
      end
    end

    it "returns errors for invalid attributes" do
      params = Avram::Params.new({"title" => "this is still good", "size" => "20", "dontcha_know" => "nope"})
      OperationWithValidations.run(params) do |operation, value|
        operation.valid?.should eq false
        value.should eq nil
        operation.errors[:size].should contain "is too high"
