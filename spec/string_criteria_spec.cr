...
describe String::Lucky::Criteria do
  ...
  describe "lower" do
    it "uses LOWER" do
      name.lower.eq("elon").to_sql.should eq ["SELECT #{QueryMe::COLUMNS} FROM users WHERE LOWER(users.name) = $1", "elon"]
    end
  end

  describe "upper" do
    it "uses UPPER" do
      name.upper.eq("elon").to_sql.should eq ["SELECT #{QueryMe::COLUMNS} FROM users WHERE UPPER(users.name) = $1", "elon"]
    end
  end
  ...
  describe "not" do
    describe "with chained criteria" do
      ...
      it "resets after having negated once" do
        name.not.like("pete").name.eq("sarah").to_sql.should eq ["SELECT #{QueryMe::COLUMNS} FROM users WHERE users.name NOT LIKE $1 AND users.name = $2", "pete", "sarah"]
        name.not.ilike("pete").name.eq("sarah").to_sql.should eq ["SELECT #{QueryMe::COLUMNS} FROM users WHERE users.name NOT ILIKE $1 AND users.name = $2", "pete", "sarah"]
      end
    end
  end
end
