...
abstract class Avram::Model
  ...
  macro serialized(type_declaration)
    column {{ type_declaration.var }}_raw : String, serialized: true
    ...
    def {{ type_declaration }}
      ...
      {% if nilable %}
      if {{ type_declaration.var }}_raw.blank? || {{ type_declaration.var }}_raw == "{}"
        nil
      else
        {{ data_type }}.from_json({{ type_declaration.var }}_raw)
      end
      {% else %}
      {{ data_type }}.from_json({{ type_declaration.var }}_raw)
      {% end %}
  ...
  macro column(type_declaration, autogenerated = false, serialized = false)
    {% if type_declaration.type.is_a?(Union) %}
      {% data_type = "#{type_declaration.type.types.first}".id %}
      {% nilable = true %}
    {% else %}
  ...
  macro setup_column_info_methods(columns, *args, **named_args)
    ...
    def self.columns : Array({name: String, nilable: Bool, type: String})
      [
        {% for column in columns %}
          {% column_name = column[:serialized] ? "#{column[:name]}::text" : column[:name] %}
          {
            name: {{ column_name.stringify }},
            nilable: {{ column[:nilable] }},
