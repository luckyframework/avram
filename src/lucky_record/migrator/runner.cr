...
class LuckyRecord::Migrator::Runner
  MIGRATIONS_TABLE_NAME = "migrations"

  extend LuckyCli::TextHelpers

  @@migrations = [] of LuckyRecord::Migrator::Migration::V1.class
  ...
  def self.create_db(quiet? : Bool = false)
    ...
    unless quiet?
      puts "Done creating #{LuckyRecord::Migrator::Runner.db_name.colorize(:green)}"
    end
  rescue e : Exception
